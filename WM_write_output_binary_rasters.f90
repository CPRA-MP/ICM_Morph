subroutine write_output_binary_rasters
    ...
    open(unit=800, file = trim(adjustL('output/'//edge_eoy_xyz_file//'.b') ),form='unformatted')
    write(800) dem_edge
    close(800)
    
    write(  *,*) ' - writing output raster binary file for topobathy DEM'
    write(000,*) ' - writing output raster binary file for topobathy DEM'
    open(unit=801, file = trim(adjustL('output/'//dem_eoy_xyz_file//'.b') ),form='unformatted')
    write(801) dem_z    
    close(801)
    ...
    open(unit=802, file = trim(adjustL('output/'//lndchng_eoy_xyz_file//'.b') ),form='unformatted')
    ...
    open(unit=803, file = trim(adjustL('output/'//lndtyp_eoy_xyz_file//'.b') ),form='unformatted')
    ...
    open(unit=804, file = trim(adjustL('output/'//dz_eoy_xyz_file//'.b') ),form='unformatted')
    write(804) dem_dz_cm
    close(804)    
    
    write(  *,*) ' - writing output raster binary file for X-coordinate'
    write(000,*) ' - writing output raster binary file for X-coordinate'
    open(unit=805, file = trim(adjustL('output/raster_x_coord.b') ),form='unformatted')
    write(805) dem_x
    close(805)  
