subroutine write_output_summaries
    ...
    integer :: write_debug                                          ! flag to indicate whether debug files are to be written (1) or not (0)
    
    write_zonal_stats = 0       ! do not write zonal stats grid and comp files - these will be written by ICM.py using
    write_debug = 0             ! do not write debug files
   ...
   &                    comp_pct_upland(i)
        ...
        ! write output CSV file at DEM resolution to be used by external zonal statistics scripts
        open(unit=910, file = trim(adjustL(dem_grid_out_summary_file) ) )
        write(910,'(A)') 'ndem,ICM_LAVegMod_GridCell,ICM_Hydro_Compartment,landtype,edge,z_NAVD88_m'
        do i = 1,ndem
            write(910,1910) i,dem_grid(i),dem_comp(i),dem_lndtyp(i),dem_edge(i),dem_z(i)
        end do
        close(910)
