...
3. To add subgrid resolution features to the inundation criteria, this code now maps the forested areas to the highest pixels of land withing each 480-m ICM-LAVegMod grid cell. If a grid cell is to be 10% forested, then the highest 25 pixels (25/256 = 10%) will be mapped as forested land. These forested pixels will *not* be subjected to the inundation criterion for land loss.
