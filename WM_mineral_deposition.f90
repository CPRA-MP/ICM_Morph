        ...
        if (dem_to_bidem(i) == dem_NoDataVal) then                                                              ! only proceed if pixel is not within barrier island domain        
            ...
            if (c /= dem_NoDataVal) then                                                                        ! for pixels that have data for ICM-Hydro compartment ID
                do mn = 1,12                                                                                    ! sum monthly deposition for open water, edge and interior areas over entire year
                    ...
                    sed_dp_ow_mons_mx = max(0.0,dem_inun_dep(i,mn) * 100.0 * ow_bd)                             ! calculate sediment deposition mass load [g/cm^2] that would match inundation depth of the open water pixel for the month
                    sed_dp_mons_mx =    max(0.0,dem_inun_dep(i,mn) * 100.0 * mn_k2)                             ! calculate sediment deposition mass load [g/cm^2] that would match inundation depth of the marsh pixel for the month
