subroutine mineral_deposition
    ...
    mn_k2 = 2.106                                                   ! mineral soil self-packing density (g/cm3) from CRMS soil data (see 2023 Wetlands Model Improvement report)
    ow_bd = 0.835                                                   ! bulk density of water bottoms (g/cm3)
    
    ! update mineral sediment deposition per unit area of interior marsh by the portion of the marsh that was inundated during each respective month
    ...
    do i = 1,ndem
        ...
        ! sum monthly deposition for open water, edge and interior areas over entire year
        sed_dp_int  = 0.0
        sed_dp_edge = 0.0
        sed_dp_wat  = 0.0        
        do mn = 1,12
            ! mineral deposition is calculated in ICM-Hydro (and read in in PREPROCESSING) in g/m^2
            ! must convert to g/cm^2
            ! [g/cm^2] = [g/m^2]*[m/100 cm]*[m/100 cm] = [g/m^2]/10000
            sed_dp_int  = sed_dp_int  + sed_dp_mi_mons_corr(c,mn)/10000.0   ! use interior deposition corrected for inundation extents
            sed_dp_edge = sed_dp_edge + sed_dp_me_mons(c,mn)/10000.0        
            sed_dp_wat  = sed_dp_wat  + sed_dp_ow_mons(c,mn)/10000.0
        end do
